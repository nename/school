...
*/
	...
	if (q == NULL){ // checks if queue isn't NULL
		queueError(QERR_INIT);
	} else {
		for (int i = 0; i < QUEUE_SIZE; i++){
			q->arr[i] = '*';
		}
		q->f_index = 0; // initialize
		q->b_index = 0;
	}
}
...
*/
	...
	return (++index % QUEUE_SIZE); // returns next index in queue
...
*/
	...
	return ((q->f_index == q->b_index) ? 1 : 0); // if the queue is empty, return 1 else 0
}
...
*/
	...
	return ((nextIndex(q->b_index) == q->f_index) ? 1 : 0); // if the queue is full, returns 1 else 0
}
...
*/
	...
	if ((queueEmpty(q)) != 0){ // checks if the queue is empty
		queueError(QERR_FRONT);
		return;
	} else {
		*c = q->arr[q->f_index]; // saves character from the first index of queue
	}
}
...
*/
	...
	if ((queueEmpty(q)) != 0){ // checks if the queue is empty
		...
		q->f_index = nextIndex(q->f_index); // sets first index as next index to remove the first one
	}
}
...
*/
	...
	if ((queueEmpty(q)) != 0){ // checks if the queue is empty
		...
		queueFront(q, c); // saves first character from queue q into c
		queueRemove(q); // removes first character from queue
	}
}
...
*/
	...
	if ((queueFull(q)) != 0){ // checks if the queue is full
		queueError(QERR_UP);
		return;
	} else {
		q->arr[q->b_index] = c; // adds character c into the first free space
		q->b_index = nextIndex(q->b_index); // sets next free index as free index
	}
}
