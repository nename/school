...
**/
	...
	tBTNodePtr act = (*RootPtr);
	tBTNodePtr post;
	bool found = FALSE;

	if (act == NULL){ // pokud neni zadny prvek
		post = NULL;
	} else { 
		while (act != NULL){ // hledam obsah
			post = act;
			if (act->Cont == Content){
				found = TRUE;
				break;
			} else if (act->Cont < Content){ // smeruji vyhledavani
				act = act->RPtr;
			} else if (act->Cont > Content){
				act = act->LPtr;
			}
		}
	}
	if (found == TRUE){ // nasel nic nedelam
		return;
	} else { // nenasel, vkladam
		tBTNodePtr newItem;

		if ((newItem = malloc(sizeof(struct tBTNode))) == NULL){
			exit(1);
		}
...
**/
	...
	while (ptr != NULL){ // prochazi dokud nenajde posledni
		SPushP(Stack, ptr);
		BTWorkOut(ptr);
		ptr = ptr->LPtr;
	}
...
**/
	...
	while(SEmptyP(&stack) == FALSE){  // projdu cely stack
	  	temp = STopPopP(&stack);
...
**/
	...
	while(SEmptyP(&stack) == FALSE){  // prochazi dokud neni stack prazdny
	  	temp = STopPopP(&stack);
	  	BTWorkOut(temp);
...
**/
	...
	while (ptr != NULL){ // dokud nenajde posledni uzel
		SPushP(StackP, ptr);
		SPushB(StackB, TRUE);
		ptr = ptr->LPtr;
	}
...
**/
	...
	while(SEmptyP(&stackP) == FALSE) {	// dokud neni prazdny prochazim
		temp = STopPopP(&stackP);
		tempBool = STopPopB(&stackB);
		
		if (tempBool == TRUE) {	// projdu pravou stranu
			SPushB(&stackB, false);
			SPushP(&stackP, temp);
...
**/
	...
	SInitP(&stack);
	Leftmost_Inorder(temp, &stack);

	while(SEmptyP(&stack) == FALSE) {	// Pouziti inorderu pro uvolneni uzlu
		temp = STopPopP(&stack);
		Leftmost_Inorder(temp->RPtr, &stack);
		free(temp);
	}
